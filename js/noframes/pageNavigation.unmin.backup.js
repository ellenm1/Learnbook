...
function getCurrentPage(){ 	 
		...
		//It means you are using a deep link or perhaps coming back from a quiz. Ignore and remove any dl params. Add "ls" parameter to query string to prevent existing local storage from being wiped.
		...
		if((typeof resumeAtLocation!="undefined")&&(resumeAtLocation!="")){
		  	ns.localStorage.set('znThisPage', resumeAtLocation);
		     resumeAtLocation = "";//wipe it once used.
		}
		else if ((typeof pItem != "undefined")&&(pItem != null)) {			
...
function getContent(params){
	...
	function loadAjaxContent(itm, itmurl, refFromBookmark){ 
				$('#content div#div6').load(itmurl+'?ts='+ts+' #content > *', function(responseText, textStatus, XMLHttpRequest) { //what follows is the callback after loading content					 				 	
							if(responseText==""){
							...
							}//end if(responseText is blank
						    else { //else response text contained some data
								...
								changeLinks(function(){  if(testing){console.log('links changed')} });//note that this happens only after content is loaded: it is part of the callback
							
								/*
								is this the first page?
								if we never yet asked if they want to resume from bookmark (run bookmarkalert, which also shows timeout warning)
								if we already asked if they want to resume just show timeout warning
								...
								*/
								...
								//  debugger;
									if(znThisPage == 0 ){ //if this is the first page	
										...
										if (typeof ms.askedbookmark == "undefined" ){  //it has not yet run bookmark alert, so run it. It will do nothing if there is no stored location, otherwise it will show the alert.
										   var currentPageArray = [];
										   currentPageArray = ns.localStorage.get('pageArray');
										   numberOfPages = currentPageArray.length;
									 
										   bookmarkAlert(znThisPage, numberOfPages);	 //offer to resume where user left off                       
										}//if (typeof ms.startTime == "undefined"		
								
									   else {  //we don't need to run bookmarkalert, just showStartupMessage()
									   if(testing){console.log('ms.askedbookmark is true, so we dont need to run bookmarkalert, just show the startup message');}
										  showStartupMessage();
										   ...
										   //or if we are not on page 0 check if we came here from a "yes" answer to a bookmark alert 
									else if(refFromBookmark){ //were we referred here from a stored bookmark? we aren't on page 0, but we just came from a bookmarkAlert, so it is effectively the first page so show Important message
									  // ms = ns.localStorage.get('moduleVars'); //refreshing ms from latest data
									  // ms.resumeAtItem = znThisPage;
									  // ns.localStorage.set('moduleVars',ms);
									   if((refFromBookmark)&&(typeof ms.showedStartupMessage == "undefined")){ 
											   if(testing){   console.log("EEESGE - referred from bookmark and haven't showed important message yet");}
											   showStartupMessage();
												//setTimeout(function(){ showStartupMessage() }, 1000);
										}
								  } //if((refFromBookmark) 
									}//if( APIOK 
		     					
		     					
		     			}//else response text contained some data
		     			
					}); //$('#content div#div6').load(itmur
		}//end function loadAjaxContent 		
		/*****---------*********/			
	 
	window.scrollTo(0, 0); //go to top of window
	...
	if(typeof ns.localStorage !="undefined"){
		ns.localStorage.set('znThisPage', itm);//store new value in local storage
			...
			ns.localStorage.set('znThisPage',itm);//store new value in local storage
			if(testing){ console.log("Waited 1 second for local storage to appear"); }
			}, 1000);
	...
	try {
   				...
   				if(testing){console.log("TRE in try catch");}	
				}
	catch (e) {
	...
	var hidetryagainmsg =   (typeof pi.hidetryagainmsg!="undefined")?pi.hidetryagainmsg:null;
		...
		else { 	//OR its not a quiz, so load the content into this page 
		       
				 
						try { 
						if(testing) {console.log('ZZDID');}
						loadAjaxContent(itm,itmurl, referredFromBookmark ); 	
						 } //try
						catch(err){
							if(testing) {console.log('XXDID');}
							console.error('SPV error is'+err);
							loadAjaxContent(itm,itmurl, referredFromBookmark);	//this will only work if the issue was timing. this is an attempt to catch timing problems, but will not solve authentication problems. probably should replace with an alert requesting reload.	
						  }		//catch(err)					
...
function showStartupMessage(){ //warning message about timeout or other warnings.
  //check for muteStartupMsg (located in pageArray.js to shut one off at a time or top of pageNavigation if you want to quickly shut them off in all modules);
  var g_muteStartupMsg = ((typeof muteStartupMsg!= "undefined")&&(muteStartupMsg==true))? true : false; 
 
 //if it is OK to show the startup message, go ahead and do it
  if((!g_muteStartupMsg)&& (typeof ms.showedStartupMessage=="undefined")){ 
       if(testing){ console.log("QWDD in showStartupMessage and ms.showedStartupMessage==undefined" );}
  
  	/* var msg = "<p><b>Avoid time-out problems:</b> Be sure to complete this module in a timely manner. Leaving a course idle or exceeding 90 minutes to complete it may cause issues with updating your final completion.</p>";
  	         msg += "<p>Before taking a break, please use the purple “Finish Later” button at bottom-left and close MLearning before stepping away.</p>";
  	         msg += "<p>If your course doesn't seem to complete at the end, don't take it again! Call the Help Desk at 936-8000 and your data will be retrieved. It is not lost.</p>";
            */
     if ((typeof customStartupMsgTitle !="undefined")&&(customStartupMsgTitle!="")){
         var modalTitle =  customStartupMsgTitle;
     }
     else{
        var modalTitle =  "<b>Avoid problems with timing out:</b>";
     }
     if ((typeof customStartupMsg !="undefined")&&(customStartupMsg!="")){
 ...
 var htmlstring = '<div class="modal" tabindex="-1" role="dialog"><div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-header">';
     ...
     htmlstring += '<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span> </button></div>';
  ...
  $('#warningModal').modal('show');
			  ...
			  ms.showedStartupMessage = true;
			  ns.localStorage.set('moduleVars',ms); 	
			  }//end if(ms.showedStartupMessage=="undefined")
			
			  

} //end function showStartupMessage()
...
function startTimeTracker(){  //should only run under scorm control - 
//timer to determine if module has been open for more than an hour - in danger of timeout in saba.

		 setInterval(function(){
			// every 10 minutes, update ms.elapsedTime
			//if ms.startTime exists, check if elapsedTime is more than an hour. If it is, show a warning
			//if it is 1.5 hours, suspend and close the module.
			var currentTime = new Date();
			var elapsedTime = currentTime-ms.startTime;  
			//console.log('setInterval ran at '+elapsedTime);
			if (elapsedTime > 60000){
			//	console.log("its been 10 mins. Saving suspend data.");
					//clearInterval();
		    ...
		    //60000   = 60 sec 1min
		  //3600000 = 60 min 1 hr
		  //5400000 = 90 min 1.5 hr
  
} //end function startTimeTracker()
...
}//end function writeKalturaPlayer //deprecated
...
function bookmarkAlert(currentPage,numberOfPages) {
	//if this is the first page, AND there is a stored bookmark location, show alert if module is longer than 15 pages (runs after determination of version)
            if(testing){console.log('XDEin bookmarkAlert'); }
 
   //Note: bookmarkAlert can be muted on a per-module basis by adding muteBookmarkAlert=true to pageArray.js 
   //      or on an across-the-board all-modules basis by adding muteBookmarkAlert=true to top of pageNavigation.js
 
    
   	var savedLocation = SCOGetValue('cmi.core.lesson_location'); 	
   	
            if(testing){	console.log('XDE in bookmarkAlert and savedLocation = ' +savedLocation + 'typeof savedLocation= '+ typeof savedLocation+ ' ms.askedbookmark= '+ ms.askedbookmark);}
         
 /*      
	          is there only one page?
	          have we already showed the bookmark alert before during this launch?
	         
	          if mutebookmarkalert is undefined - show bookmark alert
	          if mutebookmarkalert is defined and TRUE  - show bookmark alert
	          if mutebookmarkalert is defined and FALSE - don't show bookmark alert
	    */
	    
	      
 //if we have NOT showed the bookmark alert before and have more than five pages - this will eliminate some really short modules that are mostly storyline or captivate
		 if ((typeof ms.askedbookmark=="undefined")&&(numberOfPages>5)){ 
 
		  //If mutebookmarkalert does not exist in pageArray.js OR (muteBookmarkAlert EXISTS and is false)   then SHOW bookmarklert
		 if(    ( typeof muteBookmarkAlert=="undefined" )||( (typeof muteBookmarkAlert!="undefined")&&(muteBookmarkAlert==false) )     ) { 
			  
			   if ( (currentPage == 0 ) && (typeof savedLocation !="undefined")&&(savedLocation!="") ){  
					if( savedLocation > 0){
					ms.askedbookmark = true; //set flag that we have showed this alert already
					ns.localStorage.set('moduleVars',ms); 
					ms.pageArray = ns.localStorage.get('pageArray');
					var buttonTitle = ms.pageArray[savedLocation].buttonTitle;
					var pageTitle =  (typeof ms.pageArray[savedLocation].buttonTitle!="undefined")? buttonTitle : "";
					//pageTitle =  pageTitle.replace("&amp;","and");
					pageTitle = pageTitle.replace("Score &amp; Status", "Submit Score and Complete Page");
					...
					if (answer){ // if they hit OK go to that page
							...
							window.resumeAtLocation = savedLocation; 	
							var params7 = {
								itm:savedLocation, 
								referredFromBookmark:true 	 	
						}//end params    					 
					 
					   if(testing){console.log("JSIS params7.itm="+params7.itm + " params7.referredFromBookmark= "+params7.referredFromBookmark);}
						getContent(params7); 
					  }// if they answer OK get that content
					else{ //they hit cancel
						showStartupMessage();
					} //cancel
				}//if( savedLocation > 0){
			}// if( ((currentPage == 0 ) && (typeof savedLocation !="undefined")
			   else if((currentPage == 0) && (savedLocation=="")){ 
			     showStartupMessage();
			   }//else if((currentPage == 0 && (savedLocation=="") it is OK to show bookmark alert but there is no stored bookmark.
			}//muteBookmarkAlert EXISTS and is false then SHOW bookmarklert
		 //Else muteBookmarkAlert EXISTS and is TRUE then HIDE bookmarkalert	but showStartupMessage
		  else if ( (typeof muteBookmarkAlert!="undefined")&&(muteBookmarkAlert==true) ){
		     showStartupMessage();
		  }
		}//end if ((typeof ms.askedbookmark=="undefined")&&(numberOfPages>1))
		
 //else we HAVE already showed bookmark alert or numberOfPages is 1 
		 else { showStartupMessage(); }//we already showed bookmark alert or numberOfPages is 1 
		
}//end bookmarkAlert

function continueWithoutResumeLocation(){
