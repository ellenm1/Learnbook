			  ...
			  if(testing){console.log('ABA quiztype='+quiztype+' quiz='+quiz+' qindex='+ qindex+', document.location.href='+document.location.href)}
				if( trackingmode=="scorm" && APIOK() ){
					...
					if (qserver!=null){ 
						switch (qserver) {
						case 'pr': 
						qmarkServer = 'http://uhqmarkappspr1.med.umich.edu';
						break;
						 
						case 'pr2': //saba prod
						qmarkServer = 'https://mlearningquiz2.med.umich.edu';
						break;
						 
						case 'qa': //saba qa
						qmarkServer = 'https://mlearningquizdev.med.umich.edu';
						break;
						 
						case 'ts': //test quiz server was specified explicitly in page array 
						qmarkServer = 'http://uhqmarkappsts1.med.umich.edu';
						break;
						...
						switch(window.location.hostname) {
								...
								console.log('cc');
								break;
								case 'mlearning.med.umich.edu':
								qmarkServer = 'http://uhqmarkappspr1.med.umich.edu';
								console.log('dd');
								break;
								case 'mlearningtest.med.umich.edu':
								qmarkServer = 'http://uhqmarkappspr1.med.umich.edu'; //docent should use the prod quiz server unless specified in the page array explicitly
								console.log('ee');
								break;
							default:
								qmarkServer = 'https://mlearningquiz2.med.umich.edu';
					...
					if(testing){console.log('GPE currentloc= '+currentloc+ 'p3.qurl'+p3.qurl)}
...
function openSwf(swf){	//call a full window captivate like this: onclick="openSwf('captivate/captivate6.swf');"
	if ($( "#dialog-captivate2" ).length==0){$('body').append('<div id="dialog-captivate2" style="display:none; z-index: 999;"></div>')	}
   var attr = {'wmode':'transparent','scale':'showall'}
   var wheight =  $(window).height();   // returns height of browser viewport
   var wwidth = $(window).width(); 
   var capHTML = '<div id="zCaptivateContent" style="" class="span10"></div>';
   
   $( "#dialog-captivate2" ).dialog({
   							close:true,
   							closeText: "CLOSE",
							height: wheight,
							width:  wwidth,
...
var closeSpan = $("div[aria-describedby='dialog-captivate2'] span.ui-icon-closethick");
...
if ($('#closeTitle').length==0){
	...
	'<span style="float:right;margin-right:18px;font-weight:normal;font-size:small;" id="closeTitle" onmousedown=" $( \'#dialog-captivate2\' ).dialog(\'close\')">'+
	  $( "#dialog-captivate2" ).dialog("option","closeText")+ '</span>' );
	}
   
    $( "#dialog-captivate2" ).html(capHTML); 
    
    
    openModalDialog("#dialog-captivate2");	
     						
     						
    swfobject.embedSWF(swf, "zCaptivateContent",  "95%",  "99%", "9", "expressInstall.swf", attr, null, null );
						
	$("#dialog-captivate2").dialog("option", "position", {//this is the callback function
				my: "center",
				at: "center",
				of: window
			});  // $("#dialog-captivate").dialog	
