   ...
   //we are going to redefine Captivate's standard email sending script to send data to the module
	...
	if (typeof qPage.quiz!='undefined'){
	        if (parent.APIOK()){
	                   ...
	                   if (testing){ console.log("set status to incomplete and score 0, objectiveID= "+ objectiveID);
...
function checkIfTaken(){
 ...
 if (testing){  console.log( parent.data.PageArray[parent.data.znThisPage-1] )}
		  ...
		  if (typeof qPage.quiz!='undefined'){
			...
			else{  so.write("CaptivateContent");   }
}//end function checkIfTaken()



function appendEmailBody(strBody){
//alert('in appendEmailBody, strBody= '+strBody)
...
function sendMail()//converting captivates sendmail function to send data to module and lms

{
//	alert('insendMail, grtrEmailBody= '+gstrEmailBody);
...
if (testing){console.log("stripCoreData= "+stripCoreData);}
 ...
 if (testing){ console.log("In sendMail: results_array[0]="+results_array[0]+ "results_array[1]="+results_array[1]+"<br> results_array[2]="+results_array[2]+"<br> results_array[3]="+results_array[3]+"<br> results_array[4]="+results_array[4]+"<br> results_array[5]="+results_array[5]+"<br> results_array[6]="+results_array[6]+"<br> results_array[7]="+results_array[7]+"<br>results_array[8]= "+results_array[8]+"<br> results_array[9]="+results_array[9]);
	...
	bPercentScore = bPercentScore*100;
	 ...
	 if (parent.APIOK()){	
	  qPage = parent.data.PageArray[parent.data.znThisPage-1];
			...
			//what do I want to do if bScore is null after taking quiz??
			//}
	var fin=1;
...
msgDiv.style.display='block';
 if (testing){console.log("about to MarkobjectiveDone")}
   ...
   //sSession = qPage.quiz;
	...
	//alert('document.emailForm.Results.value'+document.emailForm.Results.value)
}
...
{
//alert('in padMail, strAddress='+strAddress+ ' strSubject='+ strSubject+ 'strBody= '+ strBody)
 if (!alreadyRun) {
              alreadyRun = true;
